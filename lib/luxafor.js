...
class Luxafor {
     ...
     * @private
     * @param {Function} cb - callback to execute
     * @return {Promise} - Which returns a string
     */
     ...
     */
    strobeAsync(led, red, green, blue, time, repeat) { }
    waveAsync(type, red, green, blue, repeat, speed) { }
    patternAsync(pattern, repeat) { }

    /**
     * Get the firmware version, serial number H, and serial number L from device
     * There's no additional information about the meaning of H or L
     * @return {Object} - FW_VERSION, SERIAL_NUMBER_H, SERIAL_NUMBER_L
     */
    getDeviceInfo() {
        ...
        this.writeToDevice();
        return new Promise((resolve, reject) => {
            this.readFromDevice().then((buffer) => {
                this.readFromDevice().then((buffer) => {
                    resolve(API.DEVICE_INFO(buffer));
                }).catch((err) => {
                    reject(err);
                });
            }).catch((err) => {
